...
-- Tabel: tugas_siswa, materi_pelajaran, jadwal_pelajaran & storage
-- Jalankan skrip ini di: Supabase Dashboard -> SQL Editor -> Run
-- ===================================================================

-- 0. TABEL TUGAS SISWA & NILAI (Pengumpulan & Rekap Nilai)
CREATE TABLE IF NOT EXISTS public.tugas_siswa (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    kelas VARCHAR(20) NOT NULL,            -- Contoh: 'Kelas VII', 'Kelas VIII', 'Kelas IX'
    no_absen INT NOT NULL,                 -- Nomor Urut Absen Siswa
    nama_siswa VARCHAR(255) NOT NULL,      -- Nama Lengkap Siswa
    image_url TEXT NOT NULL,               -- URL Foto Tugas di Supabase Storage
    waktu_kirim VARCHAR(50) NOT NULL,      -- Waktu Pengiriman (contoh: "08.30 WITA")
    nilai VARCHAR(20) DEFAULT NULL,        -- Nilai dari Guru (contoh: "85", "90", "A")
    catatan_guru TEXT DEFAULT NULL,        -- Catatan / Feedback Guru untuk Siswa
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW(),
    CONSTRAINT unique_siswa_per_kelas UNIQUE (kelas, nama_siswa)
);

ALTER TABLE public.tugas_siswa ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "Public Read Access" ON public.tugas_siswa;
DROP POLICY IF EXISTS "Public Insert Access" ON public.tugas_siswa;
DROP POLICY IF EXISTS "Public Update Access" ON public.tugas_siswa;
DROP POLICY IF EXISTS "Public Delete Access" ON public.tugas_siswa;

CREATE POLICY "Public Read Access" ON public.tugas_siswa FOR SELECT TO anon, authenticated USING (true);
CREATE POLICY "Public Insert Access" ON public.tugas_siswa FOR INSERT TO anon, authenticated WITH CHECK (true);
CREATE POLICY "Public Update Access" ON public.tugas_siswa FOR UPDATE TO anon, authenticated USING (true) WITH CHECK (true);
CREATE POLICY "Public Delete Access" ON public.tugas_siswa FOR DELETE TO anon, authenticated USING (true);

-- 0B. TABEL TUGAS SUSULAN
CREATE TABLE IF NOT EXISTS public.tugas_susulan (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    ...
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW(),
    CONSTRAINT unique_siswa_susulan_per_kelas UNIQUE (kelas, nama_siswa)
);

ALTER TABLE public.tugas_susulan ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "Public Read Susulan" ON public.tugas_susulan;
DROP POLICY IF EXISTS "Public Insert Susulan" ON public.tugas_susulan;
DROP POLICY IF EXISTS "Public Update Susulan" ON public.tugas_susulan;
DROP POLICY IF EXISTS "Public Delete Susulan" ON public.tugas_susulan;

CREATE POLICY "Public Read Susulan" ON public.tugas_susulan FOR SELECT TO anon, authenticated USING (true);
CREATE POLICY "Public Insert Susulan" ON public.tugas_susulan FOR INSERT TO anon, authenticated WITH CHECK (true);
CREATE POLICY "Public Update Susulan" ON public.tugas_susulan FOR UPDATE TO anon, authenticated USING (true) WITH CHECK (true);
CREATE POLICY "Public Delete Susulan" ON public.tugas_susulan FOR DELETE TO anon, authenticated USING (true);

-- Storage Bucket untuk Foto Tugas
INSERT INTO storage.buckets (id, name, public) VALUES ('tugas-siswa', 'tugas-siswa', true)
...
CREATE POLICY "Public Storage Read" ON storage.objects FOR SELECT TO anon, authenticated USING (bucket_id = 'tugas-siswa');
CREATE POLICY "Public Storage Upload" ON storage.objects FOR INSERT TO anon, authenticated WITH CHECK (bucket_id = 'tugas-siswa');
CREATE POLICY "Public Storage Update" ON storage.objects FOR UPDATE TO anon, authenticated USING (bucket_id = 'tugas-siswa');
CREATE POLICY "Public Storage Delete" ON storage.objects FOR DELETE TO anon, authenticated USING (bucket_id = 'tugas-siswa');
